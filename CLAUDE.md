...
Single-entry `create_search_assistant()` returns a compiled `StateGraph(SearchState, context_schema=ContextSchema)` with an in-memory checkpointer. Topology is **intent-agnostic at the parent level**:

```
START → understand → answer → sorcery → (END | answer for retry)
```

All four intents (`file_read | sql | chat | search`) flow through the same `answer` node (the compiled react subgraph). The intent only affects (a) the system-prompt hint baked in by `_answer_node`, and (b) which tool **bundle** the registry exposes (see Tool registry). `tavily_search` and `read_file` are registered tools, not parent-graph nodes — the LLM decides when to call them. `route_from_sorcery` is the only conditional edge: when sorcery sets `step="retry_search"` it re-enters `answer` (with a synthesized `HumanMessage` instructing the LLM to call `tavily_search` again with a new query).
...
### Intent classification (`askanswer/nodes.py`, `askanswer/_react_internals.py`)

`understand_query_node` runs a **fast local classifier first** (`_local_intent`: regex + keyword tables for file paths, SQL, search, chat). Only ambiguous inputs hit the LLM (`_intent_from_llm`, JSON-output prompt). LLM failures fall back to `_local_intent(..., fallback=True)`.

Intent is **also reclassified inside the react loop** on each fresh human turn: `_reclassify_intent` in `_react_internals.py` re-runs `_local_intent` on the latest `HumanMessage` so mid-conversation topic shifts (e.g. chat → SQL) rebind the tool bundle on the next iteration. It deliberately skips the synthetic retry `HumanMessage` from sorcery (`step == "retry_search"`) so retries don't flip intent. When extending intents, update `_INTENTS`, the relevant keyword tuple (`_SQL_KEYWORDS`, `_SEARCH_KEYWORDS`, …), and the system-prompt hint branches in `_answer_node`.
...
`_answer_node` builds an intent-specific prompt prefix (`chat` / `sql` / `file_read` each get a different system hint; `search` falls back to either the `search_results` blob or a "call `tavily_search` if needed" note) and binds only the tools in the active bundle. If `_reclassify_intent` returns a different intent than the current state, the new value is written back to `state["intent"]`.
...
Process-wide `ToolRegistry` (`get_registry()`) is the single source of truth for which tools the LLM can see. Each `ToolDescriptor` carries a `frozenset` of **bundles** (`chat | search | file_read | sql`) and `_answer_node` filters by the active intent: `registry.list(bundle=intent)`. Seeded on first use with built-in tools (`_seed_builtin`), the SQL natural-language tool (`_seed_sql`), and any live MCP tools (`refresh_mcp`).

Built-in tools (including `tavily_search` and `read_file`) are registered in **all four bundles** (`_BUILTIN_BUNDLES = ALL_BUNDLES`) so the react loop can freely chain across intents (e.g. SQL mode reading a CSV, chat mode searching the web). The shell tool is the exception: `_SHELL_BUNDLES` excludes `sql` to keep the SQL flow focused. Confirmation-required tools (currently only `gen_shell_commands_run`) set `requires_confirmation=True`; `confirmation_names()` is what `_route_from_answer` and `_tools_node` use to detect HITL routing. After connecting/disconnecting an MCP server, call `registry.refresh_mcp()` to rebuild the `mcp:*` slice.
